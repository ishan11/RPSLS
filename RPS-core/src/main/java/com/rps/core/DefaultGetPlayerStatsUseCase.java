    ...
    public List<PlayerStat> execute() {
        ...
        return gameResultRepository.findAll().stream()
                ...
                .map( x -> {
                    ...
                    int sumLizard = x.getValue().stream().mapToInt( gr -> (gr.getPlayerThrow() == Throw.LIZARD) ? 1 : 0).sum();
                    int sumSpock = x.getValue().stream().mapToInt( gr -> (gr.getPlayerThrow() == Throw.SPOCK) ? 1 : 0).sum();
                    return new PlayerStat(x.getKey(), sumWins, sumLoses, sumTies, sumRocks, sumPapers, sumScissors,sumLizard,sumSpock);
                } )
