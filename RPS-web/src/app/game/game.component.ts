...
export class GameComponent implements OnInit, OnDestroy {
  ...
  processRankedGame() {
    ...
    {
      this.mostRecentOutcome = "It is not possbile to play with the same people."  
    } 
    else 
    {
        this.gameGateway.playGame(this.rankedGameRequest).pipe(takeUntil(this._destroy)).subscribe(gameResult => {
          ...
          {
            this.mostRecentOutcome = "It's a tie";
          }
        // this.mostRecentOutcome = gameResult.outcome + " " + gameResult.player1.name;
      });
    }
